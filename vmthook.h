...
public:
	...
	inline bool HookFunctionByAddress(void* new_function, const std::uintptr_t function_address)
	{
		for (std::size_t i = 1; i <= this->total_functions; ++i)
		{
			if (this->original_vft[i] == function_address)
			{
				this->current_vft[i] = reinterpret_cast<std::uintptr_t>(new_function);
				return true;
			}
		}
